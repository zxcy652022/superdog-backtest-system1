...
class LongShortRatioData:
    ...
    def detect_extreme_ratios(
        self, df: pd.DataFrame, threshold_long: float = 0.70, threshold_short: float = 0.30
    ) -> pd.DataFrame:
        """檢測極端多空比
    ...
    def calculate_sentiment_index(self, df: pd.DataFrame, window: int = 24) -> Dict[str, Any]:
        """計算市場情緒指數
        ...
        if df.empty or len(df) < window:
            return {"sentiment_index": 0, "sentiment": "unknown", "status": "insufficient_data"}
    ...
    def analyze_divergence(self, ratio_df: pd.DataFrame, price_df: pd.DataFrame) -> Dict[str, Any]:
        """分析多空比與價格的背離
        ...
        if ratio_df.empty or price_df.empty:
            return {"divergence": False, "type": "none", "status": "insufficient_data"}
...
def fetch_long_short_ratio(
    symbol: str, interval: str = "5m", exchange: str = "binance"
) -> pd.DataFrame:
    """便捷函數：獲取多空持倉比
...
def calculate_sentiment(symbol: str, exchange: str = "binance") -> Dict[str, Any]:
    """便捷函數：計算市場情緒指數
