...
class MultiExchangeAggregator:
    ...
    def aggregate_funding_rates(
        ...
        # 按時間聚合
        aggregated = self._aggregate_by_time(
            combined,
    ...
    def _fetch_parallel(
        ...
        **kwargs
        ...
        def fetch_single_exchange(exchange: str) -> tuple:
            try:
                if data_type == 'funding_rate':
                    df = self.funding_rate.fetch(
                        symbol, start_time, end_time, exchange=exchange
                    )
                elif data_type == 'open_interest':
                    interval = kwargs.get('interval', '1h')
                    df = self.open_interest.fetch(
