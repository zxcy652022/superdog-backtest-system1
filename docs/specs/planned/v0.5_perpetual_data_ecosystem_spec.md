# SuperDog Backtest v0.5 Technical Specification
**永續合約數據生態系統 (Perpetual Contract Data Ecosystem)**

**版本**: v0.5.0  
**狀態**: 設計階段  
**優先級**: 高 (基於v0.4成功基礎)  
**預計完成**: 3-4週  

---

## 🎯 設計目標

### **核心問題**
- v0.4僅支援OHLCV基礎數據，無法進行深度永續合約分析
- 缺乏資金費率、持倉量、基差等關鍵永續合約指標
- 單一數據源限制，無法獲得完整市場圖景
- 手動數據管理，缺乏自動化數據品質控制

### **解決方案**
- **永續合約專精數據源** - 7大關鍵數據類型
- **多交易所數據聚合** - Binance/Bybit/OKX統一接入
- **實時數據流架構** - 為實盤交易做準備
- **智能數據品質控制** - 自動檢測、修復、補齊
- **互動式CLI升級** - 選單式操作體驗

---

## 📊 數據源架構設計

### **v0.5 新增數據源**

```python
class DataSource(Enum):
    # v0.4 現有
    OHLCV = "ohlcv"
    
    # v0.5 永續合約專精數據源
    FUNDING_RATE = "funding_rate"        # 資金費率 (8小時)
    OPEN_INTEREST = "open_interest"      # 持倉量
    BASIS = "basis"                      # 期現基差
    LIQUIDATIONS = "liquidations"        # 爆倉數據
    LONG_SHORT_RATIO = "long_short"      # 多空持倉比
    TOP_ACCOUNTS = "top_accounts"        # 大戶持倉數據
    VOLUME_PROFILE = "volume_profile"    # 成交量分佈
```

### **數據源詳細規格**

#### **1. 資金費率 (FUNDING_RATE)**
```python
@dataclass
class FundingRateData:
    """資金費率數據結構"""
    timestamp: pd.Timestamp      # 時間戳
    symbol: str                  # 交易對 (BTCUSDT)
    funding_rate: float          # 資金費率 (-0.01 to 0.01)
    predicted_rate: float        # 預測資金費率
    mark_price: float           # 標記價格
    index_price: float          # 指數價格
    funding_time: pd.Timestamp  # 資金費用時間
    exchange: str               # 交易所 (binance/bybit/okx)

# 數據頻率：8小時 (00:00, 08:00, 16:00 UTC)
# 歷史深度：365天
# 數據來源：Binance, Bybit, OKX
```

#### **2. 持倉量 (OPEN_INTEREST)**
```python
@dataclass
class OpenInterestData:
    """持倉量數據結構"""
    timestamp: pd.Timestamp      # 時間戳
    symbol: str                  # 交易對
    open_interest: float         # 持倉量 (USDT計價)
    open_interest_value: float   # 持倉量價值
    change_24h: float           # 24小時變化
    change_percentage: float     # 變化百分比
    exchange: str               # 交易所

# 數據頻率：5分鐘
# 歷史深度：180天
# 計算指標：OI動量、OI/市值比、OI divergence
```

#### **3. 期現基差 (BASIS)**
```python
@dataclass
class BasisData:
    """期現基差數據結構"""
    timestamp: pd.Timestamp      # 時間戳
    symbol: str                  # 交易對
    perp_price: float           # 永續合約價格
    spot_price: float           # 現貨價格
    basis: float                # 基差 (perp - spot)
    basis_percentage: float      # 基差百分比
    annualized_basis: float     # 年化基差
    exchange: str               # 交易所

# 數據頻率：1分鐘
# 歷史深度：90天
# 計算指標：基差收斂、均值回歸信號
```

#### **4. 爆倉數據 (LIQUIDATIONS)**
```python
@dataclass  
class LiquidationData:
    """爆倉數據結構"""
    timestamp: pd.Timestamp      # 爆倉時間
    symbol: str                  # 交易對
    side: str                   # 方向 (long/short)
    size: float                 # 爆倉數量
    price: float                # 爆倉價格
    value: float                # 爆倉價值 (USDT)
    exchange: str               # 交易所

# 數據頻率：即時
# 歷史深度：30天
# 聚合指標：爆倉比率、爆倉密度、市場恐慌指數
```

#### **5. 多空持倉比 (LONG_SHORT_RATIO)**
```python
@dataclass
class LongShortRatioData:
    """多空持倉比數據結構"""
    timestamp: pd.Timestamp      # 時間戳  
    symbol: str                  # 交易對
    long_account_ratio: float    # 多頭賬戶比例
    short_account_ratio: float   # 空頭賬戶比例
    long_position_ratio: float   # 多頭持倉比例  
    short_position_ratio: float  # 空頭持倉比例
    exchange: str               # 交易所

# 數據頻率：5分鐘
# 歷史深度：90天
# 逆向指標：極端比例的反轉信號
```

---

## 🏗️ 架構設計

### **1. 數據獲取層 (Data Acquisition)**

```python
class ExchangeConnector(ABC):
    """交易所連接器基底類別"""
    
    @abstractmethod
    def get_funding_rate(self, symbol: str, start_time: datetime, end_time: datetime) -> pd.DataFrame:
        """獲取資金費率數據"""
        pass
    
    @abstractmethod
    def get_open_interest(self, symbol: str, interval: str, limit: int) -> pd.DataFrame:
        """獲取持倉量數據"""
        pass
    
    @abstractmethod
    def get_liquidations(self, symbol: str, start_time: datetime, end_time: datetime) -> pd.DataFrame:
        """獲取爆倉數據"""
        pass

class BinanceConnector(ExchangeConnector):
    """Binance 永續合約數據連接器"""
    
    def __init__(self, api_key: str = None, secret_key: str = None):
        self.base_url = "https://fapi.binance.com"  # 期貨API
        self.api_key = api_key
        self.secret_key = secret_key
        self.session = requests.Session()
        
    def get_funding_rate(self, symbol: str, start_time: datetime, end_time: datetime) -> pd.DataFrame:
        """
        獲取Binance資金費率歷史
        
        API: GET /fapi/v1/fundingRate
        限制: 1000條/請求, 權重2
        """
        endpoint = "/fapi/v1/fundingRate"
        params = {
            'symbol': symbol,
            'startTime': int(start_time.timestamp() * 1000),
            'endTime': int(end_time.timestamp() * 1000),
            'limit': 1000
        }
        # 實作API調用邏輯
        
    def get_open_interest(self, symbol: str, interval: str = '5m', limit: int = 500) -> pd.DataFrame:
        """
        獲取Binance持倉量數據
        
        API: GET /fapi/v1/openInterestHist  
        間隔: 5m, 15m, 30m, 1h, 2h, 4h, 6h, 12h, 1d
        """
        endpoint = "/fapi/v1/openInterestHist"
        # 實作邏輯

class BybitConnector(ExchangeConnector):
    """Bybit 永續合約數據連接器"""
    
    def __init__(self):
        self.base_url = "https://api.bybit.com"
        
    # 實作Bybit特定的API調用

class OKXConnector(ExchangeConnector):
    """OKX 永續合約數據連接器"""
    
    def __init__(self):
        self.base_url = "https://www.okx.com"
        
    # 實作OKX特定的API調用
```

### **2. 數據標準化層 (Data Normalization)**

```python
class DataNormalizer:
    """多交易所數據標準化器"""
    
    def normalize_funding_rate(self, raw_data: pd.DataFrame, exchange: str) -> pd.DataFrame:
        """
        標準化資金費率數據格式
        
        統一欄位：timestamp, symbol, funding_rate, predicted_rate, mark_price
        時區轉換：UTC標準時間
        符號標準化：BTCUSDT格式
        """
        normalized = raw_data.copy()
        
        # 交易所特定轉換邏輯
        if exchange == 'binance':
            normalized['funding_rate'] = raw_data['fundingRate'].astype(float)
            normalized['timestamp'] = pd.to_datetime(raw_data['fundingTime'], unit='ms')
        elif exchange == 'bybit':
            # Bybit特定轉換
            pass
        
        # 統一格式驗證
        required_columns = ['timestamp', 'symbol', 'funding_rate']
        assert all(col in normalized.columns for col in required_columns)
        
        return normalized
        
    def normalize_open_interest(self, raw_data: pd.DataFrame, exchange: str) -> pd.DataFrame:
        """標準化持倉量數據"""
        # 類似實作
        pass
```

### **3. 數據存儲層 (Data Storage)**

```python
class PerpetualDataStorage:
    """永續合約數據存儲管理器"""
    
    def __init__(self, config: DataConfig):
        self.config = config
        self.base_path = config.data_root / "perpetual"
        self.setup_directories()
        
    def setup_directories(self):
        """建立數據目錄結構"""
        directories = [
            "funding_rate/binance",
            "funding_rate/bybit", 
            "funding_rate/okx",
            "open_interest/binance",
            "open_interest/bybit",
            "liquidations/binance",
            "basis/aggregated",
            "long_short_ratio/binance"
        ]
        
        for dir_path in directories:
            full_path = self.base_path / dir_path
            full_path.mkdir(parents=True, exist_ok=True)
    
    def save_funding_rate(self, data: pd.DataFrame, symbol: str, exchange: str):
        """
        保存資金費率數據
        
        文件格式：parquet (高效壓縮)
        文件命名：{symbol}_{exchange}_{start_date}_{end_date}.parquet
        分區策略：按月份分區，避免單文件過大
        """
        if data.empty:
            return
            
        # 按月份分組保存
        for month, month_data in data.groupby(data['timestamp'].dt.to_period('M')):
            file_path = (self.base_path / "funding_rate" / exchange / 
                        f"{symbol}_{month}.parquet")
            
            # 追加模式，避免覆蓋
            if file_path.exists():
                existing_data = pd.read_parquet(file_path)
                combined_data = pd.concat([existing_data, month_data]).drop_duplicates(
                    subset=['timestamp', 'symbol'], keep='last'
                )
                combined_data = combined_data.sort_values('timestamp')
            else:
                combined_data = month_data.sort_values('timestamp')
                
            combined_data.to_parquet(file_path, index=False)
            
    def load_funding_rate(self, symbol: str, exchange: str, 
                         start_date: datetime, end_date: datetime) -> pd.DataFrame:
        """載入資金費率數據"""
        data_frames = []
        
        # 計算需要的月份範圍
        start_month = start_date.replace(day=1)
        end_month = end_date.replace(day=1)
        
        current_month = start_month
        while current_month <= end_month:
            month_str = current_month.strftime('%Y-%m')
            file_path = (self.base_path / "funding_rate" / exchange / 
                        f"{symbol}_{month_str}.parquet")
            
            if file_path.exists():
                month_data = pd.read_parquet(file_path)
                data_frames.append(month_data)
                
            # 下一個月
            if current_month.month == 12:
                current_month = current_month.replace(year=current_month.year + 1, month=1)
            else:
                current_month = current_month.replace(month=current_month.month + 1)
        
        if not data_frames:
            return pd.DataFrame()
            
        combined_data = pd.concat(data_frames, ignore_index=True)
        
        # 按時間範圍篩選
        mask = (combined_data['timestamp'] >= start_date) & (combined_data['timestamp'] <= end_date)
        return combined_data[mask].sort_values('timestamp')
```

### **4. 數據品質控制層 (Data Quality Control)**

```python
class DataQualityController:
    """數據品質控制器"""
    
    def __init__(self):
        self.quality_rules = self._load_quality_rules()
        
    def _load_quality_rules(self) -> Dict[str, Dict]:
        """載入數據品質規則"""
        return {
            'funding_rate': {
                'range': (-0.1, 0.1),           # 資金費率合理範圍
                'required_columns': ['timestamp', 'symbol', 'funding_rate'],
                'null_tolerance': 0.0,           # 不允許空值
                'duplicate_check': True          # 檢查重複數據
            },
            'open_interest': {
                'range': (0, float('inf')),     # 持倉量非負
                'required_columns': ['timestamp', 'symbol', 'open_interest'],
                'null_tolerance': 0.05,          # 允許5%空值
                'monotonicity': False            # 不要求單調性
            },
            'basis': {
                'range': (-0.05, 0.05),         # 基差合理範圍
                'outlier_threshold': 3,          # 3個標準差
                'correlation_check': True        # 與現貨價格相關性檢查
            }
        }
    
    def validate_data(self, data: pd.DataFrame, data_type: str) -> Tuple[bool, List[str]]:
        """
        驗證數據品質
        
        Returns:
            (is_valid, error_messages)
        """
        errors = []
        
        if data_type not in self.quality_rules:
            return False, [f"Unknown data type: {data_type}"]
            
        rules = self.quality_rules[data_type]
        
        # 1. 檢查必要欄位
        required_cols = rules.get('required_columns', [])
        missing_cols = [col for col in required_cols if col not in data.columns]
        if missing_cols:
            errors.append(f"Missing columns: {missing_cols}")
            
        # 2. 檢查數據範圍
        if 'range' in rules and len(data) > 0:
            min_val, max_val = rules['range']
            value_col = required_cols[-1] if required_cols else data.columns[-1]
            
            if value_col in data.columns:
                out_of_range = ((data[value_col] < min_val) | (data[value_col] > max_val)).sum()
                if out_of_range > 0:
                    errors.append(f"{out_of_range} values out of range [{min_val}, {max_val}]")
        
        # 3. 檢查空值比例
        null_tolerance = rules.get('null_tolerance', 0.0)
        for col in required_cols:
            if col in data.columns:
                null_ratio = data[col].isnull().sum() / len(data)
                if null_ratio > null_tolerance:
                    errors.append(f"Column {col} has {null_ratio:.2%} null values (max: {null_tolerance:.2%})")
        
        # 4. 檢查重複數據
        if rules.get('duplicate_check', False):
            duplicates = data.duplicated(subset=required_cols[:2]).sum()  # timestamp + symbol
            if duplicates > 0:
                errors.append(f"Found {duplicates} duplicate records")
        
        # 5. 離群值檢測
        if 'outlier_threshold' in rules and len(data) > 10:
            value_col = required_cols[-1]
            if value_col in data.columns:
                z_scores = np.abs(stats.zscore(data[value_col].dropna()))
                outliers = (z_scores > rules['outlier_threshold']).sum()
                if outliers > 0:
                    errors.append(f"Found {outliers} outliers (>{rules['outlier_threshold']} std)")
        
        return len(errors) == 0, errors
    
    def clean_data(self, data: pd.DataFrame, data_type: str) -> pd.DataFrame:
        """
        清理數據
        
        - 移除重複記錄
        - 填補缺失值  
        - 修正離群值
        - 排序時間序列
        """
        if data.empty:
            return data
            
        cleaned_data = data.copy()
        rules = self.quality_rules.get(data_type, {})
        
        # 1. 移除重複記錄
        if rules.get('duplicate_check', False):
            required_cols = rules.get('required_columns', [])
            if len(required_cols) >= 2:
                cleaned_data = cleaned_data.drop_duplicates(
                    subset=required_cols[:2], keep='last'
                )
        
        # 2. 時間排序
        if 'timestamp' in cleaned_data.columns:
            cleaned_data = cleaned_data.sort_values('timestamp')
            
        # 3. 處理缺失值
        null_tolerance = rules.get('null_tolerance', 0.0)
        if null_tolerance > 0:
            # 向前填補 (forward fill)
            cleaned_data = cleaned_data.fillna(method='ffill')
            
        # 4. 離群值修正 (用中位數替換)
        if 'outlier_threshold' in rules:
            value_col = rules.get('required_columns', [])[-1]
            if value_col in cleaned_data.columns:
                z_scores = np.abs(stats.zscore(cleaned_data[value_col].fillna(0)))
                outlier_mask = z_scores > rules['outlier_threshold']
                if outlier_mask.any():
                    median_value = cleaned_data[value_col].median()
                    cleaned_data.loc[outlier_mask, value_col] = median_value
        
        return cleaned_data
```

---

## 🔄 數據管道整合

### **擴展現有 DataPipeline (v0.4)**

```python
class DataPipeline:  # 擴展現有類別
    """統一數據管道 v0.5 - 永續合約數據支援"""
    
    def __init__(self, config: DataConfig):
        self.config = config
        self.storage = PerpetualDataStorage(config)  # 新增
        self.quality_controller = DataQualityController()  # 新增
        self.connectors = {
            'binance': BinanceConnector(),
            'bybit': BybitConnector(), 
            'okx': OKXConnector()
        }
        self._cache = {}
    
    def load_strategy_data(
        self, 
        strategy: BaseStrategy, 
        symbol: str, 
        timeframe: str,
        start_date: Optional[datetime] = None,
        end_date: Optional[datetime] = None,
        exchanges: List[str] = ['binance']  # v0.5 新增
    ) -> Dict[str, pd.DataFrame]:
        """
        根據策略需求載入數據 (v0.5 擴展)
        
        新增支援：
        - 永續合約數據源
        - 多交易所數據聚合
        - 數據品質控制
        """
        requirements = strategy.get_data_requirements()
        loaded_data = {}
        
        for req in requirements:
            try:
                if req.source == DataSource.OHLCV:
                    # v0.4 現有邏輯
                    data = self._load_ohlcv(symbol, timeframe, start_date, end_date)
                    loaded_data[req.source.value] = data
                    
                elif req.source == DataSource.FUNDING_RATE:
                    # v0.5 新增：資金費率數據
                    funding_data = self._load_funding_rate_multi_exchange(
                        symbol, start_date, end_date, exchanges
                    )
                    loaded_data[req.source.value] = funding_data
                    
                elif req.source == DataSource.OPEN_INTEREST:
                    # v0.5 新增：持倉量數據  
                    oi_data = self._load_open_interest_multi_exchange(
                        symbol, start_date, end_date, exchanges
                    )
                    loaded_data[req.source.value] = oi_data
                    
                elif req.source == DataSource.BASIS:
                    # v0.5 新增：期現基差
                    basis_data = self._calculate_basis(symbol, start_date, end_date)
                    loaded_data[req.source.value] = basis_data
                    
                # ... 其他數據源
                    
            except Exception as e:
                if req.required:
                    raise DataLoadError(f"Failed to load required data {req.source.value}: {e}")
                else:
                    print(f"Warning: Failed to load optional data {req.source.value}: {e}")
        
        return loaded_data
    
    def _load_funding_rate_multi_exchange(
        self, 
        symbol: str, 
        start_date: datetime, 
        end_date: datetime,
        exchanges: List[str]
    ) -> pd.DataFrame:
        """載入多交易所資金費率數據"""
        
        all_data = []
        
        for exchange in exchanges:
            try:
                # 1. 嘗試從本地載入
                local_data = self.storage.load_funding_rate(symbol, exchange, start_date, end_date)
                
                if local_data.empty or self._need_data_update(local_data, end_date):
                    # 2. 從API獲取最新數據
                    connector = self.connectors[exchange]
                    api_data = connector.get_funding_rate(symbol, start_date, end_date)
                    
                    # 3. 數據品質控制
                    is_valid, errors = self.quality_controller.validate_data(api_data, 'funding_rate')
                    if not is_valid:
                        print(f"Data quality issues for {exchange}: {errors}")
                        api_data = self.quality_controller.clean_data(api_data, 'funding_rate')
                    
                    # 4. 保存到本地
                    if not api_data.empty:
                        self.storage.save_funding_rate(api_data, symbol, exchange)
                        local_data = api_data
                
                if not local_data.empty:
                    local_data['exchange'] = exchange
                    all_data.append(local_data)
                    
            except Exception as e:
                print(f"Failed to load funding rate from {exchange}: {e}")
                continue
        
        if not all_data:
            return pd.DataFrame()
            
        # 5. 多交易所數據聚合
        combined_data = pd.concat(all_data, ignore_index=True)
        
        # 6. 數據後處理（去重、排序、統計）
        return self._aggregate_funding_rate_data(combined_data)
    
    def _aggregate_funding_rate_data(self, data: pd.DataFrame) -> pd.DataFrame:
        """聚合多交易所資金費率數據"""
        if data.empty:
            return data
            
        # 按時間戳聚合，計算統計量
        aggregated = data.groupby(['timestamp', 'symbol']).agg({
            'funding_rate': ['mean', 'median', 'std', 'count'],
            'predicted_rate': 'mean',
            'mark_price': 'mean'
        }).round(6)
        
        # 扁平化欄位名稱
        aggregated.columns = [f'funding_{stat}' for stat in ['mean', 'median', 'std', 'count']] + \
                            ['predicted_rate_mean', 'mark_price_mean']
        
        aggregated = aggregated.reset_index()
        
        # 計算衍生指標
        aggregated['funding_divergence'] = aggregated['funding_std']  # 交易所間分歧度
        aggregated['funding_confidence'] = aggregated['funding_count'] / len(data['exchange'].unique())
        
        return aggregated.sort_values('timestamp')
```

---

## 🎮 互動式CLI升級

### **選單系統設計**

```python
class InteractiveCLI:
    """互動式CLI選單系統 v0.5"""
    
    def __init__(self):
        self.pipeline = get_pipeline()
        self.registry = StrategyRegistry()
        
    def main_menu(self):
        """主選單界面"""
        while True:
            self._clear_screen()
            self._print_header()
            self._print_main_menu()
            
            choice = input("請選擇功能 (0-9): ").strip()
            
            if choice == '1':
                self.strategy_backtest_menu()
            elif choice == '2':
                self.batch_optimization_menu()
            elif choice == '3':
                self.live_simulation_menu()
            elif choice == '4':
                self.data_management_menu()
            elif choice == '5':
                self.strategy_management_menu()
            elif choice == '6':
                self.risk_analysis_menu()
            elif choice == '7':
                self.exchange_connection_menu()
            elif choice == '8':
                self.system_settings_menu()
            elif choice == '9':
                self.help_documentation_menu()
            elif choice == '0':
                print("感謝使用 SuperDog！再見！")
                break
            else:
                print("無效選擇，請重新輸入...")
                time.sleep(1)
    
    def _print_header(self):
        """打印標題"""
        print("╔═══════════════════════════════════════════════════════════════╗")
        print("║                    SuperDog Backtest v0.5                    ║")
        print("║                 永續合約量化交易分析平台                         ║")
        print("║                                                               ║") 
        print(f"║  SSD 狀態: {'✅ 已連接' if self._check_ssd_status() else '❌ 未連接'}   ")
        print(f"║  策略數量: {len(self.registry.list_strategies())} 個")
        print(f"║  數據源: {self._get_available_data_sources()}")
        print("╠═══════════════════════════════════════════════════════════════╣")
    
    def _print_main_menu(self):
        """打印主選單"""
        print("║  1. 📊 策略回測                                               ║")
        print("║  2. ⚙️  批量參數優化                                          ║")  
        print("║  3. 🔴 實盤模擬交易                                           ║")
        print("║  4. 📁 數據管理                                               ║")
        print("║  5. 🧠 策略管理                                               ║")
        print("║  6. ⚠️  風險分析                                              ║")
        print("║  7. 🌐 交易所連線                                             ║")
        print("║  8. ⚙️  系統設置                                              ║")
        print("║  9. ❓ 幫助文檔                                               ║")
        print("║  0. 🚪 退出                                                   ║")
        print("╚═══════════════════════════════════════════════════════════════╝")
        
    def strategy_backtest_menu(self):
        """策略回測子選單"""
        self._clear_screen()
        self._print_submenu_header("策略回測")
        
        # 1. 選擇策略
        strategies = self.registry.list_strategies()
        print("可用策略:")
        for i, strategy in enumerate(strategies, 1):
            print(f"  {i}. {strategy}")
            
        strategy_choice = self._get_choice_input("選擇策略", 1, len(strategies))
        selected_strategy = strategies[strategy_choice - 1]
        
        # 2. 選擇交易對
        symbols = get_top_symbols(10)
        print("\n可用交易對:")
        for i, symbol in enumerate(symbols, 1):
            print(f"  {i}. {symbol}")
            
        symbol_choice = self._get_choice_input("選擇交易對", 1, len(symbols))
        selected_symbol = symbols[symbol_choice - 1]
        
        # 3. 選擇時間週期
        timeframes = ['1m', '5m', '15m', '30m', '1h', '4h', '1d']
        print("\n可用時間週期:")
        for i, tf in enumerate(timeframes, 1):
            print(f"  {i}. {tf}")
            
        tf_choice = self._get_choice_input("選擇時間週期", 1, len(timeframes))
        selected_timeframe = timeframes[tf_choice - 1]
        
        # 4. 動態設置策略參數
        strategy_instance = self.registry.get_strategy(selected_strategy)()
        parameters = strategy_instance.get_parameters()
        
        print("\n策略參數設置:")
        param_values = {}
        for param_name, param_spec in parameters.items():
            current_value = input(
                f"  {param_spec.description} (預設: {param_spec.default_value}): "
            ).strip()
            
            if current_value:
                param_values[param_name] = param_spec.validate(current_value)
            else:
                param_values[param_name] = param_spec.default_value
        
        # 5. 執行回測
        print(f"\n執行回測: {selected_strategy} on {selected_symbol} {selected_timeframe}")
        print("參數:", param_values)
        
        if input("確認執行? (y/N): ").lower() == 'y':
            self._run_backtest(selected_strategy, selected_symbol, selected_timeframe, param_values)
        
        input("\n按任意鍵返回主選單...")
    
    def data_management_menu(self):
        """數據管理子選單"""
        self._clear_screen()
        self._print_submenu_header("數據管理")
        
        print("1. 📥 下載歷史數據")
        print("2. 🔄 更新數據")
        print("3. 🧹 清理數據")
        print("4. 📊 數據統計")
        print("5. ✅ 數據品質檢查")
        print("0. 🔙 返回主選單")
        
        choice = input("\n請選擇操作: ").strip()
        
        if choice == '1':
            self._download_historical_data_wizard()
        elif choice == '2':
            self._update_data_wizard()
        elif choice == '3':
            self._clean_data_wizard()
        elif choice == '4':
            self._show_data_statistics()
        elif choice == '5':
            self._run_data_quality_check()
            
    def _download_historical_data_wizard(self):
        """歷史數據下載嚮導"""
        print("\n📥 歷史數據下載嚮導")
        print("=" * 50)
        
        # 選擇數據類型
        data_types = ['OHLCV', 'Funding Rate', 'Open Interest', 'Liquidations']
        print("數據類型:")
        for i, dt in enumerate(data_types, 1):
            print(f"  {i}. {dt}")
            
        type_choice = self._get_choice_input("選擇數據類型", 1, len(data_types))
        selected_type = data_types[type_choice - 1]
        
        # 選擇交易所
        exchanges = ['Binance', 'Bybit', 'OKX']
        print("\n交易所:")
        for i, ex in enumerate(exchanges, 1):
            print(f"  {i}. {ex}")
            
        exchange_choice = self._get_choice_input("選擇交易所", 1, len(exchanges))
        selected_exchange = exchanges[exchange_choice - 1].lower()
        
        # 選擇交易對
        symbol = input("交易對 (如BTCUSDT): ").strip().upper()
        
        # 選擇時間範圍
        days = int(input("下載天數 (預設30): ") or "30")
        
        # 執行下載
        print(f"\n開始下載 {selected_type} 數據...")
        print(f"交易所: {selected_exchange}")
        print(f"交易對: {symbol}")
        print(f"天數: {days}")
        
        if input("確認下載? (y/N): ").lower() == 'y':
            try:
                self._execute_data_download(selected_type, selected_exchange, symbol, days)
                print("✅ 數據下載完成!")
            except Exception as e:
                print(f"❌ 下載失敗: {e}")
        
        input("\n按任意鍵繼續...")
```

---

## 📋 實作階段規劃

### **Phase A: 核心數據源實作** (2週)

#### **Week 1: 資金費率 + 持倉量**
```python
# 實作清單
1. data/exchanges/binance_connector.py     # Binance API連接器
2. data/exchanges/bybit_connector.py       # Bybit API連接器  
3. data/exchanges/okx_connector.py         # OKX API連接器
4. data/perpetual/funding_rate.py          # 資金費率數據處理
5. data/perpetual/open_interest.py         # 持倉量數據處理
6. data/perpetual/storage.py               # 永續數據存儲
7. data/quality/controller.py              # 數據品質控制
8. tests/test_perpetual_data.py            # 永續數據測試
```

#### **Week 2: 基差 + 爆倉 + 多空比**
```python
# 實作清單  
9. data/perpetual/basis.py                 # 期現基差計算
10. data/perpetual/liquidations.py         # 爆倉數據處理
11. data/perpetual/long_short_ratio.py     # 多空比數據
12. data/aggregation/multi_exchange.py     # 多交易所聚合
13. 更新 data/pipeline.py                  # 整合到數據管道
14. tests/test_data_aggregation.py         # 聚合功能測試
```

### **Phase B: CLI升級** (1週)

#### **Week 3: 互動式界面**
```python
# 實作清單
15. cli/interactive/main_menu.py           # 主選單系統
16. cli/interactive/strategy_menu.py       # 策略回測選單
17. cli/interactive/data_menu.py           # 數據管理選單
18. cli/interactive/utils.py               # CLI工具函數
19. cli/wizards/data_download.py           # 數據下載嚮導
20. cli/wizards/backtest_config.py         # 回測配置嚮導
21. 更新 cli/main.py                       # 整合互動模式
22. tests/test_interactive_cli.py          # CLI測試
```

### **Phase C: 整合與優化** (0.5週)

#### **Week 3.5: 最終整合**
```python
# 整合清單
23. 端到端測試 - 完整數據流測試
24. 性能優化 - 數據載入效能提升
25. 文檔更新 - README, CHANGELOG, API文檔
26. 示範策略 - 基於新數據源的策略範例
```

---

## ✅ 成功指標

### **功能指標**
- [ ] **7種永續數據源** - 全部可正常載入使用
- [ ] **3個交易所支援** - Binance/Bybit/OKX數據聚合
- [ ] **數據品質控制** - 90%+數據通過品質檢查
- [ ] **互動式CLI** - 選單式操作全部功能
- [ ] **向後兼容** - v0.4所有功能正常運作

### **技術指標**
- [ ] **API響應時間** - 單次數據請求<5秒
- [ ] **存儲效率** - parquet格式壓縮比>80%
- [ ] **數據覆蓋** - 每個交易對>90天歷史數據
- [ ] **測試覆蓋率** - 新代碼85%+測試覆蓋

### **體驗指標**
- [ ] **策略開發** - 10分鐘內完成川沐策略編寫
- [ ] **數據獲取** - 3步驟完成歷史數據下載  
- [ ] **回測執行** - CLI選單式執行策略回測
- [ ] **錯誤處理** - 友好錯誤訊息和修復建議

---

## 🎯 川沐策略示範

基於新的永續數據源，川沐策略將能實現完整功能：

```python
class KawamokuAdvancedStrategy(BaseStrategy):
    """川沐進階多因子策略 - 基於永續合約數據"""
    
    def get_parameters(self):
        return {
            'funding_threshold': float_param(0.01, "資金費率閾值", 0.001, 0.05),
            'oi_momentum_period': int_param(24, "持倉量動量週期", 6, 72),
            'basis_mean_revert': bool_param(True, "基差均值回歸"),
            'liquidation_weight': float_param(0.3, "爆倉指標權重", 0.0, 1.0),
            'multi_exchange': bool_param(True, "多交易所數據聚合")
        }
    
    def get_data_requirements(self):
        return [
            DataRequirement(DataSource.OHLCV, lookback_periods=200),
            DataRequirement(DataSource.FUNDING_RATE, lookback_periods=100),
            DataRequirement(DataSource.OPEN_INTEREST, lookback_periods=200), 
            DataRequirement(DataSource.BASIS, lookback_periods=100),
            DataRequirement(DataSource.LIQUIDATIONS, lookback_periods=30),
            DataRequirement(DataSource.LONG_SHORT_RATIO, lookback_periods=100)
        ]
    
    def compute_signals(self, data: Dict[str, pd.DataFrame], params: Dict) -> pd.Series:
        """川沐多因子信號計算"""
        
        # 1. 資金費率信號
        funding = data['funding_rate']
        funding_signal = self._funding_rate_signal(funding, params['funding_threshold'])
        
        # 2. 持倉量動量信號  
        oi = data['open_interest']
        oi_signal = self._oi_momentum_signal(oi, params['oi_momentum_period'])
        
        # 3. 基差回歸信號
        basis = data['basis']
        basis_signal = self._basis_reversion_signal(basis) if params['basis_mean_revert'] else 0
        
        # 4. 爆倉恐慌信號
        liquidations = data['liquidations']
        liq_signal = self._liquidation_panic_signal(liquidations, params['liquidation_weight'])
        
        # 5. 多空比逆向信號
        long_short = data['long_short_ratio']
        contrarian_signal = self._contrarian_signal(long_short)
        
        # 6. 多因子加權合成
        combined_signal = (
            funding_signal * 0.25 +
            oi_signal * 0.25 +
            basis_signal * 0.20 +
            liq_signal * params['liquidation_weight'] +
            contrarian_signal * 0.15
        )
        
        # 7. 信號平滑和閾值處理
        smoothed_signal = combined_signal.rolling(3).mean()
        
        signals = pd.Series(0, index=combined_signal.index)
        signals[smoothed_signal > 0.6] = 1   # 強多頭信號
        signals[smoothed_signal < -0.6] = -1 # 強空頭信號
        
        return signals
    
    def _funding_rate_signal(self, funding: pd.DataFrame, threshold: float) -> pd.Series:
        """資金費率信號：極端費率的反轉信號"""
        rate_mean = funding['funding_mean'] if 'funding_mean' in funding else funding['funding_rate']
        
        # 極端正費率 -> 空頭信號 (多頭過度擁擠)
        # 極端負費率 -> 多頭信號 (空頭過度擁擠)
        signal = pd.Series(0, index=rate_mean.index)
        signal[rate_mean > threshold] = -1
        signal[rate_mean < -threshold] = 1
        
        return signal
    
    def _oi_momentum_signal(self, oi: pd.DataFrame, period: int) -> pd.Series:
        """持倉量動量信號：OI變化趨勢"""
        oi_change = oi['open_interest'].pct_change(period)
        
        signal = pd.Series(0, index=oi_change.index)
        signal[oi_change > 0.2] = 1   # OI大幅增加 -> 趨勢信號
        signal[oi_change < -0.2] = -1 # OI大幅減少 -> 反轉信號
        
        return signal
```

**這樣川沐策略就能充分利用所有永續合約數據，實現真正的多因子量化交易！** 🚀

---

*最後更新: 2025-12-07*  
*規格版本: v1.0*  
*預計開發時程: 3-4週*
