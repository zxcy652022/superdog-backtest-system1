...
def test_position_sizer_no_entry_when_size_zero():
    """Test that size <= 0 results in no entry"""
    csv_path = "data/raw/BTCUSDT_1h_test.csv"
    data = load_ohlcv(csv_path)
    
    # Use a sizer that returns 0
    zero_sizer = FixedCashSizer(cash_amount=0, fee_rate=0.0005)
    ...
    result = run_backtest(
        ...
        initial_cash=10000,
        fee_rate=0.0005,
        position_sizer=zero_sizer
    )
    
    # Should have no trades
    assert result.metrics['num_trades'] == 0, "Should have 0 trades when size is always 0"
    assert len(result.trades) == 0, "Trades list should be empty"
...
def test_sl_priority_over_tp():
    ...
    # In case of simultaneous trigger, SL should have priority
    # We can't easily test this without specific data, but we verify the logic exists
...
def test_metrics_profit_factor():
    ...
    print(f"OK test_metrics_profit_factor passed (PF={pf})")
...
def test_metrics_avg_win_loss():
    ...
    print(f"OK test_metrics_avg_win_loss passed (avg_win={avg_win}, avg_loss={avg_loss})")
...
def test_metrics_win_loss_ratio():
    ...
    print(f"OK test_metrics_win_loss_ratio passed (WL_ratio={wl_ratio})")
...
def test_metrics_expectancy():
    ...
    print(f"OK test_metrics_expectancy passed (expectancy={expectancy})")


def test_metrics_edge_cases():
    """Test metrics edge cases: 0 trades, 1 trade, all wins, all losses"""
    
    # This is conceptual - we can't easily create these scenarios
    # without custom strategies, but we verify the metrics handle them
