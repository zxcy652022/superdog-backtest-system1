...
from data.paths import get_experiments_dir
...
class ExperimentRunner:
    ...
    def _flush_results(self, experiment_id: str, runs: List[ExperimentRun]):
        ...
        Args:
            ...
            runs: 運行記錄列表
        """
        output_dir = get_experiments_dir() / "results" / experiment_id
    ...
    def save_result(self, result: ExperimentResult, output_path: Optional[str] = None):
        ...
        Args:
            result: 實驗結果
            output_path: 輸出路徑（默認為 SSD/experiments/results/{experiment_id}/）
        """
        if output_path is None:
            output_dir = get_experiments_dir() / "results" / result.experiment_id
        else:
