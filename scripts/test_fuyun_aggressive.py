...
class FuYunRollingStrategy(BaseStrategy):
    ...
    def __init__(self, broker, data, **kwargs):
        ...
        self.params = {
            ...
            "ma20_buffer": 0.02,
            # 浮雲滾倉核心參數
            "profit_threshold": 0.05,  # 利潤閾值
            "initial_leverage": 10,  # 初始槓桿
            "leverage_schedule": [10, 10, 7, 7, 5, 5, 3, 3, 2, 1],  # 槓桿遞減
            "position_size_pct": 0.10,  # 初始倉位
            "max_add_count": 10,  # 最大加倉次數
            "add_pnl_ratio": 1.0,  # 浮盈加倉比例 (1.0 = 100%)
            # 止損參數
        ...
        # 更新槓桿遞減表
        if "leverage_schedule" in kwargs:
            self.LEVERAGE_SCHEDULE = kwargs["leverage_schedule"]
        else:
            self.LEVERAGE_SCHEDULE = self.params["leverage_schedule"]
    ...
    def _check_long_entry(self, row, i) -> bool:
        ...
        if pd.isna(avg20) or not self._is_uptrend(row):
            return False
    ...
    def _check_short_entry(self, row, i) -> bool:
        ...
        if pd.isna(avg20) or not self._is_downtrend(row):
            return False
    ...
    def _check_add_position_trigger(self, current_price: float) -> bool:
        ...
        if not self.broker.has_position or self.add_count >= p["max_add_count"]:
            return False
    ...
    def _execute_add_position(self, current_price: float, current_time):
        ...
        # 使用浮盈的指定比例加倉
        add_margin = unrealized_pnl * p["add_pnl_ratio"]
    ...
    def _manage_position(self, row, current_time, bar_index: int):
        ...
        if self._check_add_position_trigger(row["close"]):
            self._execute_add_position(row["close"], current_time)
...
def run_backtest(data: pd.DataFrame, initial_cash: float, **kwargs):
    broker = SimulatedBroker(
        initial_cash=initial_cash,
        fee_rate=0.0004,
        leverage=kwargs.get("initial_leverage", 10),
        maintenance_margin_rate=0.005,
    )

    strategy = FuYunRollingStrategy(broker=broker, data=data, **kwargs)
    ...
    long_pnl = sum(t.pnl for t in trades if hasattr(t, "direction") and t.direction == "long")
    short_pnl = sum(t.pnl for t in trades if hasattr(t, "direction") and t.direction == "short")

    # 如果沒有 direction，用 qty 判斷
    if long_pnl == 0 and short_pnl == 0:
        long_pnl = sum(t.pnl for t in trades if t.qty > 0)
        short_pnl = sum(t.pnl for t in trades if t.qty < 0)
...
def main():
    ...
    configs = [
        {
            "name": "保守版 (5x, 8%閾值, 5次加倉)",
            "params": {
                "initial_leverage": 5,
                "profit_threshold": 0.08,
                "max_add_count": 5,
                "add_pnl_ratio": 0.5,
                "leverage_schedule": [5, 5, 3, 3, 2, 2, 1, 1, 1, 1],
            },
        },
        {
            "name": "平衡版 (7x, 5%閾值, 10次加倉)",
            "params": {
                "initial_leverage": 7,
        ...
        {
            "name": "激進版 (10x, 5%閾值, 無限加倉)",
            "params": {
                "initial_leverage": 10,
                "profit_threshold": 0.05,
                "max_add_count": 100,
                "add_pnl_ratio": 1.0,
                "leverage_schedule": [10, 10, 7, 7, 5, 5, 3, 3, 2, 1],
            },
        },
        {
            "name": "超激進版 (15x, 3%閾值, 無限加倉)",
            "params": {
                "initial_leverage": 15,
                "profit_threshold": 0.03,
                "max_add_count": 100,
                "add_pnl_ratio": 1.0,
                "leverage_schedule": [15, 15, 10, 10, 7, 7, 5, 5, 3, 2],
            },
        },
        {
            "name": "極限版 (20x, 3%閾值, 無限加倉)",
            "params": {
                "initial_leverage": 20,
                "profit_threshold": 0.03,
                "max_add_count": 100,
                "add_pnl_ratio": 1.0,
                "leverage_schedule": [20, 20, 15, 15, 10, 10, 7, 5, 3, 2],
            },
        },
    ]

    print("\n【策略對比 - 完整回測 2019-2025】")
    print(f"{'配置':<35} {'最終資金':>12} {'收益率':>12} {'回撤':>8} {'爆倉':>6} {'勝率':>8}")
    print("-" * 90)

    for cfg in configs:
        result = run_backtest(data.copy(), initial_cash=500, **cfg["params"])
        ret_pct = (result["final_equity"] / 500 - 1) * 100

        print(
            f"{cfg['name']:<35} {format_equity(result['final_equity']):>12} "
            f"{'+' if ret_pct >= 0 else ''}{ret_pct:>10.0f}% "
            f"{result['max_drawdown']*100:>7.1f}% {result['liquidations']:>6} "
            f"{result['win_rate']*100:>7.1f}%"
        )

    # 逐年對比最激進 vs 最保守
    print("\n" + "=" * 100)
    print("【逐年對比】激進版 (10x) vs 保守版 (5x)")
    print("=" * 100)
    ...
    for name, params in configs_compare:
        print(f"\n【{name}】")
        print(
            f"{'年份':<6} {'收益':>10} {'累計資本':>12} {'回撤':>8} {'勝率':>6} {'做多':>12} {'做空':>12} {'BH':>8}"
        )
        print("-" * 85)
        ...
        for year in years:
            ...
            result = run_backtest(year_data, initial_cash=cumulative, **params)
            year_return = (result["final_equity"] / cumulative - 1) * 100

            print(
                f"{year:<6} {'+' if year_return >= 0 else ''}{year_return:>9.0f}% "
                f"{format_equity(result['final_equity']):>12} "
            ...
            cumulative = result["final_equity"]

        print("-" * 85)
        total_return = (cumulative / 500 - 1) * 100
        print(
            f"{'總計':<6} {'+' if total_return >= 0 else ''}{total_return:>9.0f}% {format_equity(cumulative):>12}"
        )
