...
**影子交易系統 (Shadow Trading)**
- **`live/shadow_runner.py` v1.0.2** - 完整模擬交易系統
  - v2 策略：均線糾纏突破（6均線糾纏 + 放量突破）
  - 完整 USDT 權益追蹤（等同實盤計算）
  - 動態槓桿階梯：25x→20x→15x→10x→5x→3x→2x
  - 權益曲線持久化（`data/shadow_equity.json`）
  - 訊號記錄（`data/shadow_signals.json`）
  - 增強版 Telegram 通知（👻 前綴區分）

**實盤系統改進**
- **`live/multi_runner.py` v1.0.2** - 加強止損邏輯
  - 新增 `add_position_min_profit` 檢查（3% 盈利才能加倉）
  - 除以零保護
  - 盈虧計算修正（ROI% 使用 margin 而非 position_value）

- **`live/runner.py`** - 標記為已棄用（DEPRECATED）
  - 加入 DeprecationWarning
  - 建議使用 `multi_runner.py`

**專案結構整理**
- 合併 `configs/` → `config/`（統一配置目錄）
- 舊腳本移至 `scripts/archive/`（164 個）
- 實驗策略移至 `strategies/experimental/`（16 個）
- 歸檔文件移至 `docs/archive/`（23 個）

**新增文檔**
- `docs/STRATEGY_GUIDE.md` - 策略版本對照表（v1 回踩 vs v2 糾纏突破）
- `docs/STRATEGY_COMPLIANCE.md` - 策略合規檢查

**回測引擎模組化**
- `backtest/add_position.py` - 加倉邏輯模組
- `backtest/stops.py` - 止損邏輯模組
- `backtest/strategy_config.py` - 策略配置模組

### Changed

- `.pre-commit-config.yaml` - 放寬 E402 限制（允許 sys.path 設定後的 import）
- `.gitignore` - 新增臨時檔案排除

### Infrastructure

- **Linode 部署** (`172.104.52.214`)
  - multi_runner 實盤運行中（v1 回踩策略）
  - shadow_runner 影子測試中（v2 糾纏突破策略）
  - 兩系統同時監控 BTC/ETH/SOL/BNB/XRP
